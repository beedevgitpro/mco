...
addressToCoordinates(
    List<HospitalData> address, List<Placemark> coordinates) async {
  if (address.length != 0) {
...
Set<Marker> createMarker(
    List<HospitalData> placedData, BuildContext context) {
  return <Marker>[
    for(var i = 0; i < placedData.length; i++)
      Marker(
          markerId: MarkerId("${markerid++}"),
          position: LatLng(double.tryParse(placedData[i].hospitalLat),
              double.tryParse(placedData[i].hospitalLong)),
